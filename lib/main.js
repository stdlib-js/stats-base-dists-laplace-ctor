...
function Laplace() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '13h8p,HM', mu ) );
		}
		if ( !isPositive( b ) ) {
			throw new TypeError( format( '13h7c,HN', b ) );
		}
	} else {
		mu = 0.0;
		b = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '13h8d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 'b', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '13h8k,HE', value ) );
			}
			b = value;
		}
	});
